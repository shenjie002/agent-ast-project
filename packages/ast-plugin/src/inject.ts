...
export interface InjectSourceInput {
  code: string;
  filePath: string;
  route: string;
}

export interface InjectSourceResult {
  code: string;
...
export function injectSource(input: InjectSourceInput): InjectSourceResult {
  ...
  traverse(ast, {
    ...
    JSXOpeningElement(path: NodePath<t.JSXOpeningElement>) {
      ...
      const existingAttribute = getAttribute(path.node, 'data-ai-id');
      const explicitId = getStringAttribute(path.node, 'data-ai-id');
      const text = getElementText(path.parentPath.node);
      ...
      if (!existingAttribute) {
        path.node.attributes.push(
          t.jsxAttribute(t.jsxIdentifier('data-ai-id'), t.stringLiteral(elementId)),
        );
      }
...
function getAttribute(node: t.JSXOpeningElement, attributeName: string): t.JSXAttribute | undefined {
...
function getElementText(node: t.Node): string {
